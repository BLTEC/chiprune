...
// Draws one glyph at half width (4px instead of 8px) by sampling every
// other column of the existing 8x8 font, at an absolute pixel position
// (independent of the text console cursor). Height stays 8px.
void gfx_putc_small(u32 x, u32 y, char c, u32 fgcol)
{
	if (c < 32 || c > 126)
		return;

	const u8 *cbuf = &_gfx_font[8 * (c - 32)];
	for (u32 i = 0; i < 8; i++)
	{
		u8 v = cbuf[i];
		for (u32 j = 0; j < 4; j++)
		{
			if (v & (1 << (j * 2)))
				gfx_set_pixel(x + j, y + i, fgcol);
		}
	}
}

// Draws a string with gfx_putc_small, 5px pitch per char (4px glyph + 1px
// gap), honoring '\n'. Does not touch the text console cursor.
void gfx_puts_small(u32 x, u32 y, const char *s, u32 fgcol)
{
	u32 cx = x;
	for (; *s; s++)
	{
		...
		gfx_putc_small(cx, y, *s, fgcol);
		cx += 5;
	}
}

// Filled circle, small radius only (bounding-box distance test is fine
// for r <= ~6 on this resolution).
void gfx_fill_circle(u32 cx, u32 cy, u32 r, u32 color)
{
	int r2 = (int)(r * r);
...
// Ring (outline circle), used for a "pin 1" style corner marker.
void gfx_draw_ring(u32 cx, u32 cy, u32 r, u32 color)
{
	int r2o = (int)(r * r);
	int r2i = (int)((r - 1) * (r - 1));
	for (int dy = -(int)r; dy <= (int)r; dy++)
